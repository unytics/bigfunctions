...
{% if datasets|length > 1 %}    {% endif %}{% if type == 'procedure' %}call{% else %}select{% endif %} {{ dataset.name }}.{{ bigfunction.name }}({% for argument in example.arguments %}{{ argument | replace('{{ region }}', dataset.region) }}{% if not loop.last %}, {% endif %}{% endfor %}){% if type == 'procedure' %};{% elif 'output' in bigfunction %} as {{ bigfunction.output.name }}{% endif %}
